...
public class InMemoryTaskManager implements TaskManager {
    ...
    public void addSubtask(Subtask subtask) {
        ...
        if (epic == null) {
            System.out.println("Эпик с ID " + epicId + " не найден. Подзадача не может быть добавлена.");
            return; // Прерываем выполнение метода, если эпик не существует
        }

        subtask.setId(generateNextId()); // Устанавливаем ID подзадачи
        subtasks.put(subtask.getId(), subtask); // Добавляем подзадачу в хешмапу
